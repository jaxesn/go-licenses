...
import (
	...
	"github.com/jaxesn/go-licenses/internal/third_party/pkgsite/source"
...
func Libraries(ctx context.Context, classifier Classifier, ignoredPaths []string, importPaths ...string) ([]*Library, error) {
	...
	packages.Visit(rootPkgs, func(p *packages.Package) bool {
		...
		if p.Module == nil {
			...
			klog.Errorf("Package %s does not have module info. Non go modules projects are no longer supported. For feedback, refer to https://github.com/jaxesn/go-licenses/issues/128.", p.PkgPath)
			return false
		}
		...
		if isVendorModule(newModule(p.Module), pkgDir) {
			...
			// When scanning for licenses in vendor'd modules, stop upwards traversal at the vendor directory
			stopAt = path.Join(splits[0], "vendor")
		}
...
func (l *Library) FileURL(ctx context.Context, filePath string) (string, error) {
	...
	if m.Version == "" {
		...
		// * I have a module at github.com/jaxesn/go-licenses/submod.
		...
		// Examples:
		// * https://github.com/jaxesn/go-licenses/blob/HEAD/LICENSE
	...
	// https://github.com/jaxesn/go-licenses/issues/73#issuecomment-1005587408
